...
class VAEWithVIStructure(nn.Module):
    ...
    def __init__(self,
        ...
        self.vi_optimizer = vi_optimizer if vi_optimizer is not None else TreeOptimizer(latent_dim, {}, loss_callback=self.loss_callback)
    ...
    def classic_tree_reparameterize(self, mu: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        """
        ...
        ordering = self.current_vi_structure.get_ordering()  # A list of node indices (should have length == batch_size)
        device = mu.device
        ...
        for node in ordering:
            if self.current_vi_structure.get_parent(node) is None:
                ...
                path = self.current_vi_structure.get_path(node)  # e.g., [root, ..., node]
                ...
                eff_corr_root = self.current_vi_structure.compute_effective_correlation(root_node, node)
                ...
                contribution = contribution + eff_corr_root * e_dict[root_node] * sigma[node]
                ...
                if len(path) > 1:
                    for idx in range(1, len(path) - 1):
                        ...
                        eff_corr_ancestor = self.current_vi_structure.compute_effective_correlation(ancestor, node)
                        # Get gamma for edge (parent(ancestor), ancestor)
                        parent_of_ancestor = self.current_vi_structure.get_parent(ancestor)
                        if parent_of_ancestor is None:
                            gamma_val = torch.ones(latent_dim, device=device)  # default for root
                        else:
                            gamma_val = self.current_vi_structure.gamma.get((parent_of_ancestor, ancestor), torch.ones(latent_dim, device=device))
                        scaling = torch.sqrt(1.0 - gamma_val.pow(2))
                        if ancestor not in e_dict:
                            e_dict[ancestor] = torch.randn(latent_dim, device=device)
                        term = eff_corr_ancestor * scaling * e_dict[ancestor] * sigma[node]
                        contribution = contribution + term
                # Finally, set z[node] = mu[node] + sum of contributions.
                z[node] = mu[node] + contribution
        return z
    
    def fast_tree_reparameterize(self, mu: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        """        
    ...
    def tree_reparameterize(self, mu: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
        if self.faster_version:
            return self.fast_tree_reparameterize(mu, sigma)
        else:
            return self.classic_tree_reparameterize(mu, sigma)

    def _compute_reconstruction_loss(self, recon: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
    ...
    def _compute_entropy(self, sigma: torch.Tensor, tree: Optional[TreeStructure]) -> torch.Tensor:
        ...
        gamma_vals = [
            ...
            for edge in tree.edge_list if edge in tree.gamma
        ]
        edge_correction = 0.5 * torch.sum(torch.stack(gamma_vals)) if gamma_vals else 0.0
        return singleton_entropy + edge_correction


    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        batch_size = x.size(0)
        if (self.current_vi_structure is None) or (self.current_vi_structure.adj_matrix.size(0) != batch_size):
        ...
        self.current_vi_structure = self.vi_optimizer.optimize_tree(self.current_vi_structure, batch_size, self.current_elbo)
